...
export default function ReadingGapFillingQuestion({ question, answer, onAnswerChange, isPractice = false }) {
  const [gaps, setGaps] = useState({});

  // Initialize from existing answer or create empty gaps
  useEffect(() => {
    if (isPractice && answer?.answer_json) {
      try {
        const parsedData = typeof answer.answer_json === 'string' 
        ...
        console.error('[GapFillingQuestion] Error parsing practice answer:', error);
      }
    } else if (!isPractice && question.answer_data?.answer_json) {
      try {
